...
class CriticBiasCallback(EvalCallback):
    def __init__(
        ...
        # added slightly more to not run out of seeds
        seed_list = jax.random.randint(jax.random.PRNGKey(jax_random_key_for_seeds), (11000000,), 0, 2 ** 30 - 1)
        # cast to numpy
...
class EvalCallback(EventCallback):
    ...
    def __init__(
        ...
        self.jax_seed = jax_random_key_for_seeds
    ...
    def _log_success_callback(self, locals_: Dict[str, Any], globals_: Dict[str, Any]) -> None:
        ...
        if locals_["done"]:
            # different ifs, because new wrappers use 'success' name.
            if 'success' in info.keys():
                maybe_is_success = info['success']
            elif 'is_success' in info.keys():
                maybe_is_success = info['is_success']
            else:
        ...
        if self.eval_freq > 0 and (self.n_calls % self.eval_freq == 0 or self.n_calls == 1):
            ...
            if len(self._is_success_buffer) > 0:
                ...
                if self.verbose >= 1:
                    print(f"Success rate: {100 * success_rate:.2f}%")
                self.logger.record("eval/success_rate", success_rate)
                # Log success to CSV format.
                self.csv_writer.add_row(
                    {"step": self.num_timesteps, "reward": float(success_rate), "seed": self.jax_seed})
            else:
                # Log rewards to CSV format.
                self.csv_writer.add_row(
                    {"step": self.num_timesteps, "reward": float(mean_reward), "seed": self.jax_seed})
