...
class SACPolicy(BaseJaxPolicy):
    ...
    def _predict(self, observation: np.ndarray, deterministic: bool = False) -> np.ndarray:
        ...
        self.debug_log_action(observation, action, "_predict")
    ...
    def _predict_deterministic(self, observation: np.ndarray, **kwargs) -> np.ndarray:
        ...
        self.debug_log_action(observation, action, "_predict_deterministic")
        return action

    def predict_action_with_logprobs(self, observation: np.ndarray, deterministic: bool = False) -> np.ndarray:
        if deterministic:
            action = BaseJaxPolicy.select_action(self.actor_state, observation, True)
            self.debug_log_action(observation, action, "predict_action_with_logprobs")
            return action
        # Trick to use gSDE: repeat sampled noise by using the same noise key
        if not self.use_sde:
            self.reset_noise()

        action = BaseJaxPolicy.sample_action(self.actor_state, observation, self.noise_key, True)
        self.debug_log_action(observation, action, "predict_action_with_logprobs")
        return action
    ...
    def debug_log_action(self, observation, action, function_name):
        if jnp.any(jnp.isnan(action)):
            print(f"Action is None in {function_name}")
            print("Observations: ", observation)
            print("Actor state: ", self.actor_state)
            print("Action: ", action)
