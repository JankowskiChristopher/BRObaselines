...
from jaxrl.agents.sac.actor import update_quantile as update_actor_quantile
from jaxrl.agents.sac.actor import update_categorical as update_actor_categorical
from jaxrl.agents.sac.critic import target_update, validate_batch, critic_activations
from jaxrl.agents.sac.critic import update as update_critic
from jaxrl.agents.sac.critic import update_quantile as update_critic_quantile
from jaxrl.agents.sac.critic import update_categorical as update_critic_categorical
from jaxrl.datasets import Batch
from jaxrl.networks import critic_net, policies
from jaxrl.networks.common import InfoDict, Model, PRNGKey

@functools.partial(jax.vmap, in_axes=(0, 0, 0, 0, 0, 0, 0, None, None, None, 0, None, None, None, None, None, None, None, None, None, None))
def _update(
    rng: PRNGKey, actor: Model, critic: Model, target_critic: Model,
    temp: Model, pessimism: Model, batch: Batch, discount: float, tau: float, target_entropy: float, pessimism_value: jnp.ndarray, critic_regularization: int, network_regularization: int, use_sam: bool,
    distributional: bool, n_atoms: int, quantile_taus: jnp.ndarray, v_min: float, v_max: float, support: jnp.ndarray, soft_critic: bool
    ...
    return rng, new_actor, new_critic, new_target_critic, new_temp, new_pessimism, {
        **critic_info,
        **actor_info,
        **alpha_info,
        **pessimism_info,
...
def _do_multiple_updates(
    ...
    def one_step(i, state):
        step, rng, actor, critic, target_critic, temp, pessimism, info = state
        step = step + 1
        new_rng, new_actor, new_critic, new_target_critic, new_temp, new_pessimism, info = _update(
            rng,
            actor,
            critic,
            target_critic,
        ...
        return step, new_rng, new_actor, new_critic, new_target_critic, new_temp, new_pessimism, info

    step, rng, actor, critic, target_critic, temp, pessimism, info = one_step(
        0, (step, rng, actor, critic, target_critic, temp, pessimism, {})
    )
    return jax.lax.fori_loop(1, num_updates, one_step, (step, rng, actor, critic, target_critic, temp, pessimism, info))
...
class SACLearner(object):
    def __init__(
        ...
        reset_list = []
        reset_step = (2560000 // 32)
        while reset_step < 3000000:
        ...
        def _init_models(seed):
            ...
            rng, actor_key, critic_key, temp_key, pessimism_key = jax.random.split(rng, 5)
            ...
            return actor, critic, target_critic, temp, pessimism, rng

        self.init_models = jax.jit(jax.vmap(_init_models))
        self.actor, self.critic, self.target_critic, self.temp, self.pessimism, self.rng = self.init_models(self.seeds)
        self.step = 1
    ...
    def update(self, batch: Batch, num_updates: int, env_step: int) -> InfoDict:
        ...
        step, rng, actor, critic, target_critic, temp, pessimism, info = _do_multiple_updates(
            self.rng,
            self.actor,
            self.critic,
            self.target_critic,
