...
Batch2 = collections.namedtuple(
    'Batch',
    ['observations', 'actions', 'rewards', 'logprobs', 'masks', 'dones', 'next_observations'])
...
class Dataset(object):
    ...
    def sample_multiple(self, batch_size: int, n_batches: int) -> Tuple[Batch]:
        ...
        for i in range(n_batches):
            batches.append(Batch(observations=self.observations[indx[i*batch_size:(i+1)*batch_size]],
                                 actions=self.actions[indx[i*batch_size:(i+1)*batch_size]],
                                 rewards=self.rewards[indx[i*batch_size:(i+1)*batch_size]],
                                 masks=self.masks[indx[i*batch_size:(i+1)*batch_size]],
                                 next_observations=self.next_observations[indx[i*batch_size:(i+1)*batch_size]]))
