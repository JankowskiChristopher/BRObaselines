...
def weight_reinit_random(param, mask, key, weight_scaling=False, scale=1.0, weights_type="incoming"):
    ...
    new_param = nn.initializers.orthogonal(jnp.sqrt(2))(key, shape=param.shape)
    ...
    if weight_scaling:
        ...
        non_recycled_norm = jnp.sum(norm_per_neuron * (1 - neuron_mask)) / non_dead_count
    ...
    def log_intersected_dead_neurons(self, intermediates):
        ...
        if self.prev_neuron_score is None:
            ...
            for prev_k_score, current_k_score in zip(self.prev_neuron_score.items(), neuron_score_dict.items()):
                ...
                log_dict[f"mean_score_recycled/{k[:-9]}"] = float(jnp.mean(score[prev_mask]))
                log_dict[f"mean_score_nondead/{k[:-9]}"] = float(jnp.mean(score[nondead_mask]))
    ...
    def log_dead_neurons_count(self, intermediates):
        ...
        def log_dict(score, score_type):
            ...
            for k, m in score_dict.items():
                ...
                log_dict[f"dead_{score_type}_percentage/{k[:-9]}"] = (float(deadneurons_count) / layer_size) * 100.0
            ...
            log_dict[f"dead_{score_type}_percentage"] = (float(total_deadneurons) / total_neurons) * 100.0
            return log_dict
...
class NeuronRecycler(BaseRecycler):
    ...
    def is_reset(self, update_step):
        within_reset_interval = update_step >= self.reset_start_step and update_step < self.reset_end_step
    ...
    def recycle_dead_neurons(self, intermedieates, params, key, opt_state):
        ...
        (random, zero).
        """
        activations_score_dict = flax.traverse_util.flatten_dict(intermedieates, sep="/")
        ...
        reinit_fn = functools.partial(
            weight_reinit_random, weight_scaling=self.weight_scaling, scale=self.incoming_scale, weights_type="incoming"
        )
        weight_random_reset_fn = jax.jit(jax.vmap(functools.partial(jax.tree_map, reinit_fn)))
        ...
        if self.init_method_outgoing == "random":
            ...
            weight_zero_reset_fn = jax.jit(jax.vmap(functools.partial(jax.tree_map, weight_reinit_zero)))
        ...
        reset_momentum_fn = jax.jit(jax.vmap(functools.partial(jax.tree_map, reset_momentum)))
        new_mu = reset_momentum_fn(opt_state[0][1], unfreeze(incoming_mask))
        new_mu = reset_momentum_fn(new_mu, unfreeze(outgoing_mask))
        new_nu = reset_momentum_fn(opt_state[0][2], unfreeze(incoming_mask))
        new_nu = reset_momentum_fn(new_nu, unfreeze(outgoing_mask))
        ...
        return unfreeze(params), opt_state
    ...
    def create_masks(self, param_dict, activations_dict, key):
        ...
        Returns:
          ...
          param_dict
        """
        incoming_mask_dict = {k: jnp.zeros_like(p) if p.ndim != 1 else None for k, p in param_dict.items()}
        outgoing_mask_dict = {k: jnp.zeros_like(p) if p.ndim != 1 else None for k, p in param_dict.items()}
        ingoing_random_keys_dict = {k: None for k in param_dict}
        outgoing_random_keys_dict = {k: None for k in param_dict} if self.init_method_outgoing == "random" else {}

        # prepare mask of incoming and outgoing recycled connections
        for k in self.reset_layers:
            param_key = k + "/kernel"
            param = param_dict[param_key]
            # TODO: nested models are not supported?
            ...
            if isinstance(next_key, list):
                next_key = next_key[0]
            next_key = next_key + "/kernel"
            next_param = param_dict[next_key]
            ...
            next_keys = self.next_layers[k] if isinstance(self.next_layers[k], list) else [self.next_layers[k]]
            for next_k in next_keys:
                ...
                keys_and_subkeys = jax.vmap(random.split)(key)
                key, subkey = keys_and_subkeys[:, 0, :], keys_and_subkeys[:, 1, :]
        ...
        return (incoming_mask_dict, outgoing_mask_dict, ingoing_random_keys_dict, outgoing_random_keys_dict, param_dict)

    # @functools.partial(jax.jit, static_argnames=('self', 'neuron_mask'))
