...
from jaxrl.evaluation import evaluate_mw
from jaxrl.metaworld import make_env_meta
import copy
import pickle
...
flags.DEFINE_string("universe", "metaworld", "Universe name.")
...
def evaluate_if_time_to(i, agent, eval_env, eval_returns, info, seeds, save_dir):
    if i % FLAGS.eval_interval == 0:
        if i < FLAGS.start_training:
            ...
            eval_stats = evaluate_mw(agent, eval_env, FLAGS.eval_episodes, episode_length=200)
...
def main(_):
    ...
    env = make_env_meta(FLAGS.env_name, FLAGS.num_seeds, FLAGS.seed)
    eval_env = make_env_meta(FLAGS.env_name, FLAGS.num_seeds, FLAGS.seed+42)
