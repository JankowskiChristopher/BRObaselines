...
from jaxrl.inspection_utils import calculate_dormant_neurons
...
flags.DEFINE_integer("checkpoint_freq", int(10e6), "Frequency at which to save agent and buffer.")
...
flags.DEFINE_string("universe", "dm_control", "Universe name.")
...
def evaluate_if_time_to(i, agent, eval_env, eval_returns, info, seeds, save_dir):
    if i % FLAGS.eval_interval == 0:
        if i < FLAGS.start_training:
            ...
            eval_stats = evaluate(
                agent, eval_env, FLAGS.eval_episodes, episode_length=1000 // FLAGS.action_repeat
            )
...
def main(_):
    ...
    replay_buffer = ParallelReplayBuffer(
        env.observation_space, env.action_space.shape[-1], FLAGS.replay_buffer_size, num_seeds=FLAGS.num_seeds
    )

    observations, dones, rewards, infos = env.reset(), False, 0.0, {}
    start_step, agent, replay_buffer, eval_returns, update_count = restore_checkpoint_if_existing(
        save_dir, agent, replay_buffer
    )

    checkpointing_due = False

    for i in tqdm.tqdm(range(start_step, FLAGS.max_steps + 1), smoothing=0.1, disable=not FLAGS.tqdm):
        if i < FLAGS.start_training:
        ...
        replay_buffer.insert(observations, actions, rewards, masks, dones, next_observations)
        ...
        if checkpointing_due_now:
            save_checkpoint(save_dir, i, agent, replay_buffer, eval_returns, update_count)
            checkpointing_due = False
